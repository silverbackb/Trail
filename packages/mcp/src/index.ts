...
export class TrailMCP extends McpAgent<Env, Record<string, never>, Record<string, never>> {
  server = new McpServer({ name: "trail", version: "0.1.0" });

  async init() {
    // trail_get_journey — full touchpoint path for a lead
    this.server.registerTool(
      "trail_get_journey",
      {
        description: "Get the full multi-touch journey (all sessions) for a specific lead. Returns each touchpoint in chronological order with channel, source, and timestamp.",
        inputSchema: {
          lead_id:    z.string().describe("The lead ID from your CRM or form submission"),
      ...
      async ({ lead_id, account_id }) => {
        ...
        return {
          content: [{
            type: "text",
            text: `Journey for lead ${lead_id} (${rows.results.length} touchpoints):\n\n${lines.join("\n\n")}`,
          }],
        };
      }
    );

    // trail_get_report — attribution report by model
    this.server.registerTool(
      "trail_get_report",
      {
        ...
        inputSchema: {
          account_id: z.string().describe("Your Trail account ID"),
          model: z.enum(["first_touch", "last_touch", "linear"]).default("last_touch").describe("Attribution model to use"),
        },
      },
      async ({ account_id, model }) => {
        ...
        return {
          content: [{
            type: "text",
            text: `Attribution report — model: ${model}\n\n${"Channel".padEnd(20)} Leads        Conversions\n${"─".repeat(55)}\n${lines.join("\n")}`,
          }],
        };
      }
    );

    // trail_get_top_paths — most common conversion paths
    this.server.registerTool(
      "trail_get_top_paths",
      {
        description: "Get the most common multi-touch paths taken by leads before converting. Useful to understand which channel sequences drive the most business.",
        inputSchema: {
          account_id: z.string().describe("Your Trail account ID"),
      ...
      async ({ account_id, limit }) => {
        ...
        return {
          content: [{
            type: "text",
            text: `Top ${limit} conversion paths:\n\n${paths.join("\n")}`,
          }],
        };
      }
    );
    ...
    this.server.registerTool(
      ...
      {
        description: "Get channel performance breakdown: leads, conversions, and conversion rate per channel. Use to identify which channels drive real business vs just traffic.",
        inputSchema: {
          account_id: z.string().describe("Your Trail account ID"),
        },
      },
...
export default {
  fetch(request: Request, env: Env, ctx: ExecutionContext) {
    return TrailMCP.serve("/mcp", { binding: "MCP_AGENT" }).fetch(request, env, ctx);
  },
};
